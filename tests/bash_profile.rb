...
include FileHelper  
 ...
 def init
   self.name = "Bash Profile Checks"
   self.description = "Checks for existence of .bash_profile file(s), and data within."
   self.filename = File.basename(__FILE__, ".rb")
 end
 
 def run
   files = instance.runcmd_pty('sudo find / -type f -name ".bash_profile"')
   unless (files.to_s.empty?)
      unless (files.to_s =~ /No such/i)
	 files.to_s.split(/[\r\n]+/).each { |file|
